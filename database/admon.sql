...
CREATE TABLE IF NOT EXISTS `mydb`.`valuaciones` (
  `articulos_id` INT ZEROFILL NOT NULL,
  `peritos_id` INT NOT NULL,
  `monto` FLOAT NOT NULL,
  `fecha` DATE NOT NULL,
  `numero_dictamen` CHAR(255) NOT NULL,
  CONSTRAINT `fk_valuaciones_articulos`
    FOREIGN KEY (`articulos_id`)
    REFERENCES `mydb`.`articulos` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE INDEX `fk_articulos_has_peritos_articulos1_idx` ON `mydb`.`valuaciones` (`articulos_id` ASC);
